...
XLMR_DIR = 'xlm-roberta-base'
MINILM_DIR = 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2'
E5_DIR = 'intfloat/multilingual-e5-base'
...
def get_args_parser():
    ...
    parser.add_argument('--text_model', default='minilm', type=str)
    # Training
...
class Trunk(nn.Module):
    def __init__(self, device, model) -> None:
        ...
        self.clip_extractor, self.clip_processor = get_clip_extractor() # openai clip
         # korean - english
        self.text_extractor, self.text_processor = get_text_extractor(model)
    ...
    #     features[ModalityType.ENG_TEXT]  = self.get_eng_text_feature(input[ModalityType.TEXT])
    #     features[ModalityType.KOR_TEXT]  = self.get_eng_text_feature(input[ModalityType.KOR_TEXT])
    ...
    def get_text_feature(self, inputs: [str]) -> Tensor:
        # inputs = self.clip_processor(text=texts, max_length=77, truncation=True, return_tensors="pt", padding='max_length')
    ...
    def get_eng_text_feature(self, inputs) -> Tensor:
        ...
        # elif self.model == 'xlmr' or self.model == 'e5':
    ...
    def get_kor_text_feature(self, inputs) -> Tensor:
        # inputs = self.text_processor(texts, return_tensors="pt", padding=True, truncation=True)
        outputs = self.text_extractor(**inputs)
        pooled = mean_pooling(outputs, inputs['attention_mask'])
        # elif self.model == 'xlmr' or self.model == 'e5':
...
def get_text_extractor(model):
    if model == 'minilm':
        return AutoModel.from_pretrained(MINILM_DIR), AutoTokenizer.from_pretrained(MINILM_DIR)
    elif model == 'xlmr':
        return AutoModel.from_pretrained(XLMR_DIR), AutoTokenizer.from_pretrained(XLMR_DIR)
    elif model == 'e5':
        return AutoModel.from_pretrained(E5_DIR), AutoTokenizer.from_pretrained(E5_DIR)
