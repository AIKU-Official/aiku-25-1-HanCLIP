...
# Load the dataset
streamed_dataset = load_dataset("QuoQA-NLP/KoCC3M", split="validation", streaming=True, trust_remote_code=True)
...
for i in range(len(samples)):
    image = Image.open(BytesIO(requests.get(samples[i]['image_url']).content)).convert("RGB")
    text = samples[i]['korean_caption']
    images.append(image)
    texts.append(text)
